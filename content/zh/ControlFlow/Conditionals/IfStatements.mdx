...
The program checks the value of the `age` <GlossaryTerm term="variable">variable</GlossaryTerm>. If `age < 13` is true, it runs the first block. If `age >= 13` is true, it runs the second block. Here, age is 15, so the first block is skipped and the second runs.
