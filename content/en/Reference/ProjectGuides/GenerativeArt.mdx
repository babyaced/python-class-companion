...
<Checklist>
  ...
  <Checkbox id="ga-s4-2" label="Have Canvas store shapes in a list. render(filename) should take a filename as a parameter, loop through the list calling draw() on each shape, then save the image to that file." />
