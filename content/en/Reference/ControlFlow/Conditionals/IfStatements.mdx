...
Python checks `age < 13` first. If that's false, the person is 13 or older, so `elif age < 18` correctly catches 13–17. Everyone else (18+) hits the `else`. The order of the conditions does the work.
...
- Use `else` for "otherwise" and `elif` when you have several cases and want to pick one.
- The condition is an expression that's true or false. On the **Comparison Operators** and **Logical Operators** pages you'll see how to write and combine conditions. You'll use the same ideas in **loops**, which run a block of code multiple times.

## Try it out

<PythonRunner code={`age = 15
if age < 13:
    print("Recommend family-friendly movies")
elif age < 18:
    print("Recommend teen movies")
else:
    print("Recommend adult movies")
`} />
