...
In this example, the `match` statement checks the value of the `day` <GlossaryTerm>variable</GlossaryTerm> against each `case`.
...
As you code, you'll recognize situations where using `match` and `case` can make your code clearer than using multiple conditional statements, 
but generally speaking, `match` is particularly useful when you have a variable that can take on many different values and you want to handle each value differently.

## Try it out

<PythonRunner code={`day = "Monday"
